 ...
 *
 * Returns: SVG image (1200×630) with tree/staff data + S33D branding.
 */
...
function svgDefs(hasPhoto: boolean): string {
  return `<defs>
    ...
    ${hasPhoto ? `<clipPath id="photoClip"><rect x="0" y="0" width="640" height="630" rx="0"/></clipPath>` : ""}
  </defs>`;
}
...
function treeCardSVG(d: TreeCardData): string {
  ...
  // Layout: photo left half, text right half (or full-width text if no photo)
  ...
  ${hasPhoto ? `
  <!-- Photo (left half) -->
  <image href="${esc(d.photoUrl!)}" x="0" y="0" width="640" height="630" clip-path="url(#photoClip)" preserveAspectRatio="xMidYMid slice"/>
  <!-- Photo fade overlays -->
  <rect x="0" y="0" width="640" height="630" fill="url(#photoFade)"/>
  <rect x="0" y="0" width="640" height="630" fill="url(#photoFadeBottom)"/>
  ` : `
  <!-- No-photo decorative element -->
  ...
  <text x="320" y="330" text-anchor="middle" fill="${MUTED}" font-family="serif" font-size="72" opacity="0.12">🌳</text>
  `}
  ...
  <text x="${textX}" y="${hasPhoto ? 325 : 345}" fill="${GOLD}" font-family="'Helvetica Neue','Arial',sans-serif" font-size="14" opacity="0.6" letter-spacing="0.5">
    Minted with ${esc(d.staffLabel)}
  </text>` : ""}

  <!-- S33D branding -->
  ${brandingBlock(textX, hasPhoto ? 530 : 530)}
...
function staffCardSVG(d: StaffCardData): string {
  ...
  ` : `
  <circle cx="320" cy="315" r="180" fill="none" stroke="${GOLD}" stroke-width="0.5" stroke-opacity="0.08"/>
  <text x="320" y="335" text-anchor="middle" fill="${MUTED}" font-family="serif" font-size="72" opacity="0.12">⚡</text>
  `}
...
async function fetchTreeData(id: string): Promise<TreeCardData> {
  try {
    ...
    const [treeRes, photoRes, mintRes, staffRes] = await Promise.all([
      ...
      supabase.from("ceremony_logs").select("staff_name, staff_code").eq("ceremony_type", "binding").limit(1).maybeSingle(),
    ]);
    ...
    return {
      ...
      staffLabel: staffRes.data?.staff_name || staffRes.data?.staff_code || null,
      photoUrl: photoRes.data?.media_url || null,
    };
  } catch {
    return { name: "Ancient Friend", species: "Unknown species", location: "Unknown location", isMinted: false, staffLabel: null, photoUrl: null };
  }
}
...
function resolveStaffData(code: string): StaffCardData {
  ...
  return { code, species, isOrigin: !isCircle, photoUrl: imgPath };
}
...
Deno.serve(async (req) => {
  ...
  if (type === "staff") {
    ...
    svg = treeCardSVG(await fetchTreeData(id));
  }
