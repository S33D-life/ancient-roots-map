...
-- Fix: schema-qualify digest() call to extensions.digest() so it resolves
-- regardless of search_path. Without this, every real draw fails.
...
BEGIN
  ...
  -- Schema-qualified digest() so it works regardless of search_path
  v_seed := encode(
    extensions.digest(
  ...
  SELECT p_draw_id, user_id, SUM(amount)::integer
