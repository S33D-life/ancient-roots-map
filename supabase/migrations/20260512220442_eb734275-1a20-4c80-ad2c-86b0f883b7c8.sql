...
BEGIN
  ...
  SELECT
    ...
    COALESCE((SELECT COUNT(*) FROM public.trees t WHERE t.added_by = u.id), 0)::bigint AS trees_added,
    COALESCE((SELECT COUNT(*) FROM public.offerings o WHERE o.user_id = u.id), 0)::bigint AS offerings_count,
...
REVOKE ALL ON FUNCTION public.admin_users_list(text, int, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_users_list(text, int, boolean) TO authenticated;