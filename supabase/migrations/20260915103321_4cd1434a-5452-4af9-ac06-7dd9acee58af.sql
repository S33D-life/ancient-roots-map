...
DECLARE v_uid uuid := auth.uid(); p record; keep record; keepj jsonb; drop_id uuid; dropped record;
        ...
        v_folded jsonb := '{}'::jsonb; v_ids uuid[];
...
BEGIN
  ...
  -- A person may hold only one "first encounter" record per tree. Where the same
  -- person first met both records, the superseded encounter is carried over under
  -- its own mark rather than stranded: its reflection, photo and date travel with it.
  -- No reward trigger fires on this relabelling (hearts follow canopy proof and
  -- insertion only), so nothing is rewarded twice.
  ...
  -- Guardianship is one row per person, tree and role. Where the same person already
  -- guards the surviving record in the same way, their standing is folded in: the
  -- earlier beginning and the combined tending count are kept.
  ...
  -- A wish is one row per person and tree, but it may carry a written note.
  -- Fold the note into the surviving wish so no words are stranded.
  ...
  -- Name whatever still rests with the superseded record, so nothing vanishes quietly.
...
REVOKE EXECUTE ON FUNCTION public.approve_tree_merge(uuid, uuid, jsonb, text, boolean) FROM anon;