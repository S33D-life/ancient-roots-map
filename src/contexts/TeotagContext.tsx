...
function buildSummary(
  ...
  if (pageCtx.staff) {
    ...
    if (s.speciesHeartsEarned) parts.push(`Species Hearts earned: ${s.speciesHeartsEarned}`);
    if (s.councilSessions) parts.push(`Council sessions attended: ${s.councilSessions}`);
  }
  ...
  if (pageCtx.hive) {
    const h = pageCtx.hive;
    parts.push(`Viewing Species Hive: ${h.speciesName || h.family}`);
    if (h.treeCount) parts.push(`Trees in hive: ${h.treeCount}`);
    if (h.speciesHearts) parts.push(`Hive Species Hearts: ${h.speciesHearts}`);
    if (h.memberCount) parts.push(`Active members: ${h.memberCount}`);
    if (h.vitalityLevel) parts.push(`Hive vitality: ${h.vitalityLevel}`);
  }

  if (pageCtx.spiral) {
    const sp = pageCtx.spiral;
    parts.push(`Spiral view: ${sp.viewMode || "overview"}`);
    if (sp.selectedSpecies) parts.push(`Selected species: ${sp.selectedSpecies}`);
    if (sp.staffCount) parts.push(`Staff count visible: ${sp.staffCount}`);
  }
