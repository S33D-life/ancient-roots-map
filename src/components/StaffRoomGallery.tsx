...
export default function StaffRoomGallery() {
  ...
  // ── View mode toggle ──────────────────────────────────────────
  const ViewToggle = () => (
    <div className="flex items-center gap-1 rounded-lg p-1" style={{ background: "hsl(var(--secondary) / 0.5)" }}>
      {([
  ...
  const ListView = () => (
    <div className="space-y-2">
      {filteredStaffs.map((staff, i) => (
        <div
          key={staff.tokenId}
          className="flex items-center gap-4 p-3 rounded-lg border border-border/40 bg-card/50 hover:bg-card/80 cursor-pointer transition-all group"
  ...
  const GalleryView = () => (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      {filteredStaffs.map((staff, i) => (
        <Card
          key={staff.tokenId}
          className="border-border/40 hover:border-primary/50 transition-all group cursor-pointer overflow-hidden hover:shadow-[var(--glow-subtle)]"
          onClick={() => { setActiveIndex(i); setDetailOpen(true); }}
          role="button"
          aria-label={`View ${staff.speciesName} staff`}
        >
  ...
  const FullScreenView = () => (
    <motion.div
      ...
      style={{
        background: "radial-gradient(ellipse at 30% 70%, hsl(28 25% 10% / 0.95), hsl(80 15% 6% / 0.98))",
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 z-10">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-full bg-card/50 backdrop-blur text-foreground hover:bg-card/80 transition-colors"
            aria-label="Exit full screen"
          >
            <X className="w-4 h-4" />
          </button>
          <span className="font-serif text-sm text-muted-foreground">
            {activeIndex + 1} / {filteredStaffs.length}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              showFilters ? "bg-primary/20 text-primary" : "bg-card/50 text-foreground hover:bg-card/80"
            )}
        ...
        {showFilters && (
          ...
          >
            {FILTER_OPTIONS.map((f) => (
              <button
                key={f.value}
                onClick={() => { setFilter(f.value); setActiveIndex(0); }}
                className={cn(
                  "px-3 py-1.5 rounded-full text-xs font-serif whitespace-nowrap transition-all",
                  filter === f.value
                    ? "bg-primary text-primary-foreground"
                    : "bg-card/60 text-muted-foreground hover:text-foreground backdrop-blur"
                )}
              >
                {f.label}
              </button>
      ...
      <div className="flex-1 flex items-center justify-center relative px-4">
        ...
        {activeIndex > 0 && (
          <button
            onClick={() => navigate(-1)}
            className="absolute left-2 md:left-6 p-3 rounded-full bg-card/40 backdrop-blur text-foreground hover:bg-card/70 transition-all z-10"
        ...
        <AnimatePresence mode="wait">
          {activeStaff && (
            <motion.div
              key={activeStaff.tokenId}
              className="flex flex-col items-center max-w-sm w-full cursor-pointer"
            ...
            >
              <div
                className="w-full max-w-[280px] md:max-w-[320px] aspect-[3/4] rounded-xl overflow-hidden border-2 border-primary/20 shadow-2xl"
                style={{ boxShadow: "0 0 60px hsl(var(--primary) / 0.15), 0 25px 50px rgba(0,0,0,0.4)" }}
              >
                <img
                  src={activeStaff.image}
                  alt={`${activeStaff.speciesName} staff`}
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="mt-5 text-center">
                <h3 className="font-serif text-2xl text-primary tracking-wide">
                  {activeStaff.speciesName}
                </h3>
                <p className="text-sm text-muted-foreground font-mono mt-1">
                  {activeStaff.code} · #{String(activeStaff.tokenId).padStart(3, "0")}
                </p>
                {activeStaff.length && (
                  <p className="text-xs text-muted-foreground mt-1">
                    {activeStaff.length} · {activeStaff.weight}
                  </p>
                )}
              </div>
            </motion.div>
        ...
        {activeIndex < filteredStaffs.length - 1 && (
          <button
            onClick={() => navigate(1)}
            className="absolute right-2 md:right-6 p-3 rounded-full bg-card/40 backdrop-blur text-foreground hover:bg-card/70 transition-all z-10"
      ...
      {/* Bottom bar with spiral mini-map */}
      <div className="flex items-center justify-center pb-4 px-4 gap-4">
        {filter === "all" || filter === "origin" ? (
          <SpiralMiniMap
            currentIndex={activeStaff?.isOrigin ? activeStaff.index : -1}
            totalOrigin={36}
            onSelect={(i) => {
              const idx = filteredStaffs.findIndex((s) => s.index === i);
          ...
          <div className="flex gap-1">
            {filteredStaffs.map((_, i) => (
              <button
                ...
                aria-label={`Go to staff ${i + 1}`}
              />
            ))}
          </div>
        )}
        <p className="text-[10px] text-muted-foreground font-serif hidden md:block">
          ← → navigate · i details · esc exit
        </p>
      </div>
    </motion.div>
  );
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => setShowMinting(!showMinting)}
            >
      ...
      {/* Detail drawer — Sheet from right (desktop) / bottom (mobile) */}
      <Sheet open={detailOpen} onOpenChange={setDetailOpen}>
        <SheetContent side={isMobile ? "bottom" : "right"} className={cn(
          "overflow-y-auto",
          isMobile ? "max-h-[85vh] rounded-t-2xl" : "w-[400px]"
        )}>
          <SheetHeader>
            <SheetTitle className="font-serif text-xl text-primary">
              {activeStaff?.speciesName || "Staff Details"}
            </SheetTitle>
          </SheetHeader>
          {activeStaff && (
            <div className="mt-4">
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-background/80 backdrop-blur-sm" onClick={() => setOnChainStaff(null)}>
          <div className="max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto rounded-xl border border-primary/40 bg-card shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-end p-2">
              <button onClick={() => setOnChainStaff(null)} className="p-1 rounded-full hover:bg-secondary transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
