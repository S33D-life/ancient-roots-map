/**
 * EconomyCompass — Deep distribution mechanics for the Value Tree.
 * Contains: Distribution Compass, 4 Channels, Patron Flow, Emission Curve,
 * Value Branches, Council Influence layer, and canonical notes.
 * Extracted from EconomyOverview to reduce Overview tab scroll depth.
 */
...
const EconomyCompass = () => {
  ...
  useEffect(() => {
    supabase
      ...
      .select("amount")
      .then(({ data }) => {
        setMintedTotal((data || []).reduce((s, r) => s + (r.amount || 0), 0));
      });
  }, []);
  ...
  return (
    <div className="space-y-10">
      ...
      >
        ...
        <DistributionCompass mintedTotal={mintedTotal} />
      </motion.section>
      ...
      {/* ═══ Council of Life — Influence Layer ═══ */}
      <motion.section
        initial={{ opacity: 0, y: 16 }}
        animate={{ opacity: 1, y: 0 }}
