...
/** Rooms that activate map companion mode */
const MAP_ROOMS = new Set(["map", "tree"]);
...
export default function CompanionController() {
  ...
  const isMapMode = MAP_ROOMS.has(fallbackState.room);
  ...
  useEffect(() => { if (paired) hapticSuccess(); }, [paired]);

  const sendWithFeedback = useCallback(
    (cmd: Parameters<typeof sendCommand>[0]) => {
      sendCommand(cmd);
      const label =
        cmd.type === "zoom_in" ? "Zoom +" :
        cmd.type === "zoom_out" ? "Zoom −" :
        cmd.type === "zoom_reset" ? "Reset" :
        cmd.type === "pan" ? null :
        cmd.type === "map_pan" ? null :
        cmd.type === "map_zoom" ? null :
        cmd.type === "locate_me" ? "Locating…" :
        cmd.type === "next" ? "Next tree" :
        cmd.type === "previous" ? "Prev tree" :
        ...
        cmd.type === "drag_start" ? "Drag" :
        cmd.type === "drag_move" ? null :
        cmd.type === "drag_end" ? null :
        cmd.type === "send_to_desktop" ? "Sent" :
        null;
  ...
  const toggleDrag = () => {
    if (dragging) { sendCommand({ type: "drag_end" }); setDragging(false); }
      ...
      default:
        return (
          <div className="p-3 text-center">
            <p className="text-xs text-muted-foreground font-serif">Use room buttons to navigate.</p>
          </div>
        );
    }
  };
        ...
        <div className="flex items-center gap-2">
          ...
          {isMapMode && (
            <span
              ...
              style={{
                background: "hsl(120 20% 25% / 0.3)",
                color: "hsl(120 25% 60%)",
                border: "1px solid hsl(120 20% 35% / 0.3)",
              }}
            >
              Map
            </span>
          ...
          {paired && (
            <button
              ...
              className={cn(
                ...
                showDebug ? "bg-primary/20 text-primary" : "text-muted-foreground/30",
              )}
            >
          ...
          {!session ? (
            ...
            <motion.div key="join" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <div className="w-full max-w-xs flex flex-col items-center gap-5">
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Enter the 6-character code shown on your desktop.</p>
                </div>
                <div className="w-full rounded-xl p-3 space-y-1.5" style={{ background: "hsl(var(--secondary) / 0.15)", border: "1px solid hsl(var(--border) / 0.15)" }}>
                  <p className="text-[10px] uppercase tracking-widest text-muted-foreground/60 font-serif">How it works</p>
                  <ul className="text-xs text-muted-foreground space-y-1 list-none">
                    <li className="flex items-start gap-2"><span>📱</span> Phone becomes a remote</li>
                    <li className="flex items-start gap-2"><span>🗺</span> Explore the map from here</li>
                    <li className="flex items-start gap-2"><span>🎯</span> Tap trees · scroll pages · guide the view</li>
                  </ul>
                </div>
                <div className="w-full flex flex-col gap-3">
                  <Input value={codeInput} onChange={(e) => setCodeInput(e.target.value.toUpperCase())} maxLength={6} placeholder="ABCD23"
                    className="text-center text-2xl font-mono tracking-[0.3em] h-14 uppercase" autoFocus inputMode="text" autoCapitalize="characters" autoComplete="off"
                    onKeyDown={(e) => e.key === "Enter" && handleJoin()} />
                  {error && <p className="text-xs text-destructive text-center">{error}</p>}
                  <button onClick={handleJoin} className="w-full py-3 rounded-xl text-sm font-serif min-h-[48px] bg-primary text-primary-foreground hover:bg-primary/90 active:scale-[0.98] transition-all">Connect</button>
                </div>
              </div>
            </motion.div>
          ) : !paired ? (
            ...
            <motion.div key="connecting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="flex flex-col items-center gap-4">
                <div className="p-4 rounded-full" style={{ background: "hsl(var(--primary) / 0.08)", border: "1px solid hsl(var(--primary) / 0.15)" }}>
                  {pairTimedOut ? <AlertCircle className="w-8 h-8 text-destructive" /> : <Wifi className="w-8 h-8 text-primary animate-pulse" />}
                </div>
                {pairTimedOut ? (
                  <>
                    <div className="text-center">
                    ...
                    <button onClick={() => { disconnect(); setError(""); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-serif min-h-[44px] bg-secondary/40 border border-border/30 text-foreground">
            ...
            <motion.div key="controller" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <div className="flex items-center justify-center gap-3 py-1.5 relative">
                ...
                <span className="text-[10px] text-muted-foreground/50 font-serif">{ROOM_LABELS[fallbackState.room] ?? "Home"}</span>
                <AnimatePresence>
                  {lastAction && (
              ...
              <div className="flex-1 flex flex-col items-center justify-center max-w-sm mx-auto w-full min-h-0 px-4">
                <div className="w-full max-h-[28vh] overflow-y-auto">{renderController()}</div>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 px-4 pb-1">
                ...
                {mode !== "scroll" && (
                  <button onClick={toggleDrag}
                    className={cn("flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-serif min-h-[32px] transition-all",
                      dragging ? "text-foreground" : "text-muted-foreground/50")}
                    style={{
                      background: dragging ? "hsl(42 60% 55% / 0.15)" : "hsl(var(--secondary) / 0.15)",
                      border: dragging ? "1px solid hsl(42 60% 55% / 0.4)" : "1px solid hsl(var(--border) / 0.1)",
                    }}>
              ...
              <div className="px-4 pb-1.5">
                <PointerPad send={sendCommand} mode={mode} settings={settings} dragging={dragging} isMapMode={isMapMode} debug={showDebug} />
              </div>
