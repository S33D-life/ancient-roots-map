...
import { Sparkles, Camera, Music, FileText, MessageSquare, Mic, BookOpen, Heart } from "lucide-react";
    ...
    <section className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        {Object.entries(typeCounts).map(([type, count]) => (
          <Badge key={type} variant="outline" className="text-[10px] font-serif gap-1 capitalize border-border/40">
      ...
      {recent.length > 0 ? (
        ...
        >
          {recent.map((off) => (
            ...
            >
              <Card className="bg-card/50 backdrop-blur border-border/30 hover:border-primary/20 transition-colors">
        ...
        <div
          className="rounded-xl border border-dashed border-primary/20 p-8 text-center"
          style={{ background: "radial-gradient(ellipse at 50% 80%, hsl(var(--primary) / 0.04), transparent 70%)" }}
        >
          <p className="text-muted-foreground font-serif text-sm">
            No offerings yet. Be the first to share a memory with {treeName}.
          </p>
        </div>
      )}
      ...
      <div className="text-center">
        <Button
          ...
          variant="outline"
          className="font-serif tracking-wider gap-2 border-primary/30 hover:bg-primary/10 pulse-cta"
        >
