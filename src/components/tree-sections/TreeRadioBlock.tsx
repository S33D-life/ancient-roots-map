...
import { Radio, Play, Pause, Volume2, VolumeX, Music } from "lucide-react";
...
const TreeRadioBlock = ({ treeId, treeName, species, radioTheme }: Props) => {
  const [song, setSong] = useState<{ title: string; artist: string | null; previewUrl: string | null } | null>(null);
      ...
      if (data && data.length > 0) {
        ...
        try {
          ...
          if (json.results?.length) {
            setSong({ title: json.results[0].trackName, artist: json.results[0].artistName, previewUrl: json.results[0].previewUrl });
          } else {
            setSong({ title: s.title, artist: s.content, previewUrl: s.media_url });
          }
        } catch {
          setSong({ title: s.title, artist: s.content, previewUrl: s.media_url });
        }
